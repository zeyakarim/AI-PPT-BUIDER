...
type TextProps = { text: string; options?: any };
type TextPropsOptions = any;
...
export function generatePptx(presentationData: Presentation): PptxGenJS {
    ...
    presentationData.slides.forEach((slide) => {
        ...
        const raw = Array.isArray(slide.content)
            ? slide.content
            : [slide.content];
        ...
        raw.forEach((contentBlock) => {
            ...
            } else if (typeof contentBlock === "string" && contentBlock.trim().length > 0) {
                finalContentArray.push(contentBlock.trim());
            }
        });

        if (finalContentArray.length === 0 && raw.length > 0) {
            finalContentArray = raw
                .filter((l): l is string => typeof l === "string" && l.trim().length > 0)
                .map((l) => l.trim());
        }

        const bulletProps: TextProps[] = finalContentArray.map((line) =>
            bulletLine(line, 1)
        );

        if (bulletProps.length > 0) {
